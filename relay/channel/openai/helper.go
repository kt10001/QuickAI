...
func HandleFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, lastStreamData string,
	...
	// 膨胀用户可见的usage副本（不修改原始usage指针，避免影响上层扣费计算）
	displayUsage := *usage
	groupRatio := info.PriceData.GroupRatioInfo.GroupRatio
	if groupRatio > 1 {
		displayUsage.PromptTokens = int(float64(displayUsage.PromptTokens) * groupRatio)
		displayUsage.CompletionTokens = int(float64(displayUsage.CompletionTokens) * groupRatio)
		displayUsage.TotalTokens = displayUsage.PromptTokens + displayUsage.CompletionTokens
		displayUsage.PromptTokensDetails.CachedTokens = int(float64(displayUsage.PromptTokensDetails.CachedTokens) * groupRatio)
		displayUsage.CompletionTokenDetails.ReasoningTokens = int(float64(displayUsage.CompletionTokenDetails.ReasoningTokens) * groupRatio)
		if displayUsage.PromptCacheHitTokens > 0 {
			displayUsage.PromptCacheHitTokens = int(float64(displayUsage.PromptCacheHitTokens) * groupRatio)
		}
		if displayUsage.InputTokens > 0 {
			displayUsage.InputTokens = int(float64(displayUsage.InputTokens) * groupRatio)
		}
		if displayUsage.OutputTokens > 0 {
			displayUsage.OutputTokens = int(float64(displayUsage.OutputTokens) * groupRatio)
		}
	}
	...
	case types.RelayFormatOpenAI:
		if info.ShouldIncludeUsage && !containStreamUsage {
			response := helper.GenerateFinalUsageResponse(responseId, createAt, model, displayUsage)
	...
	case types.RelayFormatClaude:
		...
		info.ClaudeConvertInfo.Usage = &displayUsage
