...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		if info.ShouldIncludeUsage {
			...
			// 膨胀用户可见的usage（不影响 claudeInfo.Usage，后者用于扣费）
			groupRatio := info.PriceData.GroupRatioInfo.GroupRatio
			if groupRatio > 1 {
				openAIUsage.PromptTokens = int(float64(openAIUsage.PromptTokens) * groupRatio)
				openAIUsage.CompletionTokens = int(float64(openAIUsage.CompletionTokens) * groupRatio)
				openAIUsage.TotalTokens = openAIUsage.PromptTokens + openAIUsage.CompletionTokens
				openAIUsage.PromptTokensDetails.CachedTokens = int(float64(openAIUsage.PromptTokensDetails.CachedTokens) * groupRatio)
				openAIUsage.CompletionTokenDetails.ReasoningTokens = int(float64(openAIUsage.CompletionTokenDetails.ReasoningTokens) * groupRatio)
				if openAIUsage.PromptCacheHitTokens > 0 {
					openAIUsage.PromptCacheHitTokens = int(float64(openAIUsage.PromptCacheHitTokens) * groupRatio)
				}
				if openAIUsage.InputTokens > 0 {
					openAIUsage.InputTokens = int(float64(openAIUsage.InputTokens) * groupRatio)
				}
				if openAIUsage.OutputTokens > 0 {
					openAIUsage.OutputTokens = int(float64(openAIUsage.OutputTokens) * groupRatio)
				}
			}
...
func HandleClaudeResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, httpResp *http.Response, data []byte) *types.NewAPIError {
	...
	case types.RelayFormatOpenAI:
		...
		groupRatio := info.PriceData.GroupRatioInfo.GroupRatio
		if groupRatio > 1 {
			openaiResponse.Usage.PromptTokens = int(float64(openaiResponse.Usage.PromptTokens) * groupRatio)
			openaiResponse.Usage.CompletionTokens = int(float64(openaiResponse.Usage.CompletionTokens) * groupRatio)
			openaiResponse.Usage.TotalTokens = openaiResponse.Usage.PromptTokens + openaiResponse.Usage.CompletionTokens
			openaiResponse.Usage.PromptTokensDetails.CachedTokens = int(float64(openaiResponse.Usage.PromptTokensDetails.CachedTokens) * groupRatio)
			openaiResponse.Usage.CompletionTokenDetails.ReasoningTokens = int(float64(openaiResponse.Usage.CompletionTokenDetails.ReasoningTokens) * groupRatio)
			if openaiResponse.Usage.PromptCacheHitTokens > 0 {
				openaiResponse.Usage.PromptCacheHitTokens = int(float64(openaiResponse.Usage.PromptCacheHitTokens) * groupRatio)
			}
			if openaiResponse.Usage.InputTokens > 0 {
				openaiResponse.Usage.InputTokens = int(float64(openaiResponse.Usage.InputTokens) * groupRatio)
			}
			if openaiResponse.Usage.OutputTokens > 0 {
				openaiResponse.Usage.OutputTokens = int(float64(openaiResponse.Usage.OutputTokens) * groupRatio)
			}
		}
