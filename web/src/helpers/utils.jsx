...
export function getSystemName() {
  ...
  if (!system_name) return 'Quick AI';
  return system_name;
}
...
export const calculateModelPrice = ({
  ...
  if (record.quota_type === 0) {
    ...
    // 定价页面展示官方原价，不乘 GroupRatio（加价通过 token 膨胀隐式实现）
...
export const getModelPriceItems = (
  ...
  if (priceData.isPerToken) {
    ...
    const currencyLabel = quotaDisplayType === 'CNY' ? ' CNY' : quotaDisplayType === 'CUSTOM' ? '' : ' USD';
    const unitSuffix = `${currencyLabel} / 1${priceData.unitLabel} Tokens`;
    return [
      {
        key: 'input',
        label: t('输入价格'),
  ...
  const fixedCurrencyLabel = quotaDisplayType === 'CNY' ? ' CNY' : quotaDisplayType === 'CUSTOM' ? '' : ' USD';
  return [
    {
      key: 'fixed',
      label: t('模型价格'),
      value: priceData.price,
      suffix: `${fixedCurrencyLabel} / ${t('次')}`,
    },
  ].filter((item) => item.value !== null && item.value !== undefined && item.value !== '');
};
