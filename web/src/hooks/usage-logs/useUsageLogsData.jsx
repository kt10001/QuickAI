...
export const useLogsData = () => {
  ...
  const getDefaultColumnVisibility = () => {
    return {
      ...
      [COLUMN_KEYS.RAW_PROMPT]: isAdminUser,
      [COLUMN_KEYS.RAW_COMPLETION]: isAdminUser,
  ...
  const getInitialVisibleColumns = () => {
    ...
    try {
      ...
      if (!isAdminUser) {
        ...
        merged[COLUMN_KEYS.RAW_PROMPT] = false;
        merged[COLUMN_KEYS.RAW_COMPLETION] = false;
      }
    ...
    allKeys.forEach((key) => {
      if (
        (key === COLUMN_KEYS.CHANNEL ||
          ...
          key === COLUMN_KEYS.RAW_PROMPT ||
          key === COLUMN_KEYS.RAW_COMPLETION) &&
        !isAdminUser
      ) {
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (isAdminUser && (logs[i].type === 0 || logs[i].type === 2 || logs[i].type === 5 || logs[i].type === 6)) {
        ...
        expandDataLocal.push({ key: t('分组倍率'), value: groupRatio });
        if (modelRatio !== undefined) {
          expandDataLocal.push({ key: t('模型倍率'), value: modelRatio });
        }
        expandDataLocal.push({ key: t('补全倍率'), value: completionRatio });
        expandDataLocal.push({ key: t('真实输入 tokens'), value: logs[i].prompt_tokens });
        expandDataLocal.push({ key: t('真实输出 tokens'), value: logs[i].completion_tokens });
        if (groupRatio > 1) {
          expandDataLocal.push({ key: t('用户显示输入'), value: Math.round(logs[i].prompt_tokens * groupRatio) });
          expandDataLocal.push({ key: t('用户显示输出'), value: Math.round(logs[i].completion_tokens * groupRatio) });
        }
        if (other?.cache_tokens > 0) {
          const hitRate = ((other.cache_tokens / (logs[i].prompt_tokens || 1)) * 100).toFixed(1);
          expandDataLocal.push({ key: t('缓存命中率'), value: `${hitRate}%` });
        }
      }
      if (logs[i].type === 2) {
        expandDataLocal.push({
