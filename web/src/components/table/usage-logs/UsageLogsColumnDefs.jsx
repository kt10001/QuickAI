  ...
  return [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        // 用户看到膨胀后的token，管理员看真实值
        const rawTokens = parseInt(text) || 0;
        const displayTokens = isAdminUser ? rawTokens : Math.round(rawTokens * groupRatio);
        const cacheReadDisplay = isAdminUser ? cacheSummary?.cacheReadTokens : Math.round((cacheSummary?.cacheReadTokens || 0) * groupRatio);
        const cacheWriteDisplay = isAdminUser ? cacheSummary?.cacheWriteTokens : Math.round((cacheSummary?.cacheWriteTokens || 0) * groupRatio);

        let cacheText = '';
        if (hasCacheRead && hasCacheWrite) {
          cacheText = `${t('缓存读')} ${formatTokenCount(cacheReadDisplay)} · ${t('写')} ${formatTokenCount(cacheWriteDisplay)}`;
        } else if (hasCacheRead) {
          cacheText = `${t('缓存读')} ${formatTokenCount(cacheReadDisplay)}`;
        } else if (hasCacheWrite) {
          cacheText = `${t('缓存写')} ${formatTokenCount(cacheWriteDisplay)}`;
        }
        ...
        return record.type === 0 ||
          ...
          >
            ...
            {isAdminUser && groupRatio > 1 ? (
              ...
              >
                {t('用户')}:{Math.round(rawTokens * groupRatio)}
              </span>
            ) : null}
            {cacheText ? (
              <span
